...
import { Briefcase, ChevronDown, ChevronUp, MapPin, Eye } from 'lucide-react';
import { resumeData } from '../../data/resume';
import { cn } from '../../lib/utils';
import { ChurnDashboard } from '../projects/ChurnDashboard';

export const Experience: React.FC = () => {
  const [expandedIndex, setExpandedIndex] = useState<number | null>(0);
  const [isChurnDashboardOpen, setIsChurnDashboardOpen] = useState(false);
    ...
    <section id="experience" className="py-24 px-4 max-w-5xl mx-auto">
      ...
      <div className="space-y-6">
        {resumeData.work.map((item, index) => {
          const isDrdo = item.company.includes("DRDO");
                        ...
                        <div className="md:col-span-2 space-y-4">
                          ...
                          {isDrdo && (
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                setIsChurnDashboardOpen(true);
                              }}
                              className="mt-4 px-6 py-3 rounded-2xl bg-blue-500/10 hover:bg-blue-500 text-blue-400 hover:text-white border border-blue-500/20 hover:border-blue-500 font-bold text-xs transition-all flex items-center gap-2"
                            >
                              <Eye className="w-4 h-4" />
                              View Churn Analysis Dashboard
                            </button>
                          )}
                        </div>
      ...
      <ChurnDashboard 
        isOpen={isChurnDashboardOpen} 
        onClose={() => setIsChurnDashboardOpen(false)} 
      />
    </section>
  );
};
